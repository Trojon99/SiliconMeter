...
快速样本约每 2 秒记录一次，慢速样本约每 6 秒一次；Network RX/TX 与快速样本共用时间戳、质量和实际测量窗口字段。约每 30 秒批量写入，并在正常退出时刷盘。异常退出可能丢失最近尚未提交的一批数据。弹出窗口提供记录状态、数据库大小和**打开数据文件夹**按钮。普通 SQLite 工具可读取已提交历史；参见[v4 表结构、单位与只读查询](docs/HISTORY_SCHEMA.md)。历史数据库会持续增长。最终候选版本在 30 分钟内的 SQLite 逻辑大小增加了 344,064 字节，约合 0.688 MB/小时。按这一短期速率推算，1 小时约 0.688 MB、24 小时约 16.52 MB、7 天约 115.61 MB、30 天约 495.45 MB。此前持久文件的估计值为 0.698 MB/小时。这些是估算值，不是容量限制；运行时 WAL 和 SHM 文件还会额外占用临时磁盘空间。可通过**打开数据文件夹**找到 SQLite 文件。v0.1 不自动删除或降采样历史数据；保留策略留待后续版本。

## 隐私

所有遥测和历史数据只保存在本机。Network 监控仅读取系统接口计数器，不产生监控流量，也不主动测速。App 没有分析服务、云服务或遥测上传，不按进程归属网络流量，也不收集用户文件内容。数据库保存系统指标和 App 运行元数据，不记录工作负载标签。语言与主要指标选择存放在本地 `UserDefaults`，不写入遥测数据库。
