...
**Network presentation order.** The menu bar now shows Upload / TX / `↑` first and Download / RX / `↓` second; the popover shows Upload above Download in English and 上传 above 下载 in Chinese. The menu examples `NET ↑ 0 B/s ↓ 0 B/s`, `NET ↑ 727.6 KB/s ↓ 52.8 MB/s`, and `NET ↑ 1.0 GB/s ↓ 999.9 MB/s` passed formatter and real AppKit status-button checks. The bilingual popover smoke injected distinct TX and RX values and verified their labels, values, and row order. All 88 real status-bar cases passed fixed width, neighbor stability, and clipping checks; Network remained 249 pt and scalar metric widths and gaps matched the preceding evidence. This was a presentation-order change only. No telemetry or database semantics were changed: `ifi_obytes` still feeds TX/Upload and `network_tx_bytes_per_sec`; `ifi_ibytes` still feeds RX/Download and `network_rx_bytes_per_sec`. Network slot widths, spacing, formatter, sampler, cadence, SQLite schema, and scalar menu layouts were not changed.
